...
class data_processor:
    ...
    def create_slice_dataset(self,dataset_size = [25,5,15]):
        ...
        create_folder('Dataset/train')
        create_folder('Dataset/train/png')
        create_folder('Dataset/train/mask')
        create_folder('Dataset/valid')
        create_folder('Dataset/valid/png')
        create_folder('Dataset/valid/mask')
        create_folder('Dataset/test')
        create_folder('Dataset/test/png')
        create_folder('Dataset/test/mask')
        if len(os.listdir('Dataset/train/png')) | len(os.listdir('Dataset/train/mask')) | len(os.listdir('Dataset/valid/png')) | len(os.listdir('Dataset/valid/mask')) | len(os.listdir('Dataset/test/png')) | len(os.listdir('Dataset/test/mask')):
            print('create dataset fail! it seems that you already have some data in <Dataset> please check!\n<Error: data already exist!>')
            print('-----------------------------------------')
    ...
    def get_data(self):
        ...
        train_png_list = os.listdir('Dataset/train/png')
        print('-----------------------------------------')
        print('getting data: train')
        for train_png in tqdm(train_png_list):
            train_png_path = f'Dataset/train/png/{train_png}'
            train_mask_path = f'Dataset/train/mask/{train_png}'
            ...
            train_list.append([img_resize.rotate(random_rotate), mask_resize.rotate(random_rotate),slice_num]) # 样本三，随机旋转
            train_list.append([img_resize.transpose(Image.FLIP_LEFT_RIGHT), mask_resize.transpose(Image.FLIP_LEFT_RIGHT),slice_num]) # 样本四，左右翻转
            train_list.append([img_resize.transpose(Image.FLIP_TOP_BOTTOM), mask_resize.transpose(Image.FLIP_TOP_BOTTOM),slice_num]) # 样本五，上下翻转

        valid_png_list = os.listdir('Dataset/valid/png')
        print('-----------------------------------------')
        print('getting data: valid')
        for valid_png in tqdm(valid_png_list):
            valid_png_path = f'Dataset/valid/png/{valid_png}'
            valid_mask_path = f'Dataset/valid/mask/{valid_png}'
    ...
    def get_oldest_model_name(self):
        ...
        return 'model_save/' + str(model_name) + '.pth'
    ...
    def dataset_mean_dice_score(self,net, dataset='test'):
        ...
        for png_dir in tqdm(all_dir):
            real_mask_dir = f'Dataset/{dataset}/mask/{png_dir}'
...
class nn_processor:
    ...
    def train(self,net,lr=0.01,EPOCH=40,max_iter=500,save_iter=500,plot_iter=2000,first_iter=0,loss_func = nn.BCEWithLogitsLoss(),loss_func2 = nn.MSELoss()):
        ...
        for epoch in range(EPOCH):
            ...
            for step, (x, y,y2) in enumerate(self.train_loader):
                ...
                loss = loss_func(output1, y) * 30 + loss_func2(output2,y2).to(torch.float) * 0.01
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                i += 1
                ...
                if i % 100 == 0:
                    ...
                    if i == max_iter:  # 达到最大迭代，保存模型
                        stop = True
                        torch.save(net.state_dict(), f'model_save/{i+first_iter}.pth')
                        print('model saved!')
                        break
                    ...
                    #     plt.ylabel('loss')
                    #     plt.legend()  # 显示图例文字
                    #     plt.title(f'{i+first_iter} iteration loss')
                    #     plt.savefig(f'loss/loss {i+first_iter}.png')
                    #     print(f'-----{i+first_iter} loss figure saved!-----')
                    #     plt.close()
                    #     loss_train_list = list()  # 更新列表
                    #     loss_valid_list = list()
                    #     iter_list = list()
...
if __name__ == '__main__':
    ...
    def view_model_effect_change(patient,slice):
        ...
        for number in number_list:
            if number==86600:
                ...
                Mydata_processor.view2plot(predict_mask,real_mask,f'{number}iter myo:{round(myo,2)} lv:{round(lv,2)} rv:{round(rv,2)}',f'patient{patient} slice{slice}')
    ...
    def view_valid_dice_change(model1,model2 = 'MAX', step = 1):
        create_folder('valid dice score plot')
        if model2 == 'MAX':
